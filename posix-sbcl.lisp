...
(defun decode-stat (name stat)
  (let* ((kind (stat-kind stat))
    ...
    (apply #'make-instance kind
	   ...
	   :dev (sb-posix:stat-mtime stat)
	   :gid (sb-posix:stat-mtime stat)
	   :uid (sb-posix:stat-mtime stat)
	   :ino (sb-posix:stat-mtime stat)
	   :mode (sb-posix:stat-mtime stat)
	   :nlink (sb-posix:stat-mtime stat)
	   :size (sb-posix:stat-mtime stat)
	   rdev)))
